...
// 	아이디 유효성 체크
	const idInput = $('#userId_');
	const checkId = () => {
		if(idInput.val().trim().length < 4)
		{
	...
	const pwInput = $('#password_');
	const pwCkInput = $('#password2');
	const checkPwCk = () => {
		if(pwInput.val().trim() != pwCkInput.val().trim())
		{
	...
	$(() => {
		idInput.on('blur', checkId)
		pwCkInput.on('blur', checkPwCk)
	});
